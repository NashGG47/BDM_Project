...
def run_consumer_job():
    ...
    try:
        subprocess.run(["python", "data_ingestion/hot_paths/consumer/consume_renfe_incident.py"], check=True)
...
def run_producer_job():
    ...
    try:
        subprocess.run(["python", "data_ingestion/hot_paths/producer/process_renfe_incidents.py"], check=True)
    except subprocess.CalledProcessError as e:
